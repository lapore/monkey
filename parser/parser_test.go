...
func TestLetStatements(t *testing.T) {
    ...
    if len(program.Statements) != 3 {
        t.Fatalf("program.Statements does not containt 3 statements. got=%d",
...
func testLetStatment(t *testing.T, s ast.Statement, name string) bool {
    ...
    if !ok {
        t.Errorf("s not *ast.LetStatment type, got=%T", s)
        return false
    }
...
func TestReturnStatements(t *testing.T) {
    ...
    for _, stmt := range program.Statements {
        ...
        if !ok {
            t.Errorf("stmt not *ast.ReturnStatement type. got %T", stmt)
            continue
        }
